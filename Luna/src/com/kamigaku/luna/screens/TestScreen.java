...
public class TestScreen implements Screen {
	...
	private boolean contactBotLeft, contactBotRight, contactTopLeft, contactTopRight;
	...
	@Override
	public void show() {
		levelDatas = new Level("map/map.tmx", -9.8f);
		this.player = levelDatas.player;
		this.falling = this.jumping = this.toLeft = this.toRight = this.wantToJump = this.againstWall = false;
		this.contactBotLeft = this.contactBotRight = this.contactTopLeft = this.contactTopRight = false;
	...
	private void mouvementPlayer() {
		    ...
		    for(Contact contact : contacts) {
		    	if(contact.getFixtureA().equals(player.playerBody.getFixtureList().get(0))) {
		    		if(!againstWall)
		    			bodyIsWall(contact.getFixtureB().getBody());
		    	}
		    	if(contact.getFixtureB().equals(player.playerBody.getFixtureList().get(0))) {
		    		if(!againstWall)
		    			bodyIsWall(contact.getFixtureA().getBody());
		    	}
		    }
	...
	private void bodyIsWall(Body body) {
	...
	private void getPositionContact() {
		...
		for(Contact contact : contacts) {
			if(contact.getFixtureA().getBody().getPosition() == this.player.getTargetPos()) {
				...
				System.out.println(contact.getFixtureA().getShape().getRadius());
			}
		}
		if(positionPlayer.y + this.player.sizeY < positionObstacle.y) {
